(function () {
  ...
  var axisWidth = 0,
    ...
    .ticks(d3.time.days, 3) // How often column label is rendered
    .tickFormat(monthDayFormat), // How column label is rendered
    yAxisScale = d3.scale.linear()
    .range([0, axisHeight])
    .domain([0, 24]),
    yAxis = d3.svg.axis()
    .orient('left') // Row label position
    .ticks(24) // How many row labels will be rendered
  ...
  d3.json('json/pm.json', function (err, data) {
    data = data.data;
    ...
    rect = heatmap.selectAll('rect')
      ...
      .attr('x', function (d) {
        console.log('x', dayFormat(d.date) - dayOffset, itemSize * (dayFormat(d.date) - dayOffset));
  ...
  function renderColor() {
    ...
    rect
      ...
      .attrTween('fill', function (d, i, a) {
        ...
        console.log('colorIndex', colorIndex(d.value['PM2.5']));
